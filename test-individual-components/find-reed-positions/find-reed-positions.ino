...
void transform_read(int reedMap[BITMAP_SIZE][BITMAP_SIZE]) {
    int copy[BITMAP_SIZE][BITMAP_SIZE];
    copy_map(reedMap, copy);
    for (int i = 0; i < BITMAP_SIZE; ++i) {
        for (int j = 0; j < BITMAP_SIZE; ++j) {
            BitmapPosition twoDPos = flatTo2d(muxToReedSwitchMap[i][j]);
            reedMap[twoDPos.x][twoDPos.y] = copy[i][j];
        }
    }
}
...
BitmapPosition getActiveBit(int reedMap[BITMAP_SIZE][BITMAP_SIZE]) {
...
int reedMap[BITMAP_SIZE][BITMAP_SIZE] = {0};

void loop() {
    if (millis() + INTERVAL_MILLIS > last_time) {
        read_reed_switches(reedMap);
        BitmapPosition pos = getActiveBit(reedMap);
        if (pos.x != -1 || pos.y != -1) {
            Serial.printf("Mux Channel: %d\n", pos.x * BITMAP_SIZE + pos.y);
        }
    }
}