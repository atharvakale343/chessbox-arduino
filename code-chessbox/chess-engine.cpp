...
const char *get_piece_name(Piece piece) {
    switch (piece) {
        ...
        case BORDER:
            ...
            break;
        case NO_PIECE:
            Serial.printf("ERROR: NO_PIECE SHOULD NOT BE REACHABLE IN SWITCH CASE\n");
            return "  NO_PIECE ";
            break;
        default:
            Serial.printf("ERROR: UNREACHABLE SWITCH CASE\n");
...
int get_move_value(Move move) {
    ...
    if (getPieceColor(move.piece) == NO_COLOR) {
        Serial.printf("ERROR: move.piece was not initialized\n");
    }
...
void add_move_into_moves(Move valid_moves[], int *valid_moves_len, int valid_moves_cap, Move move) {
    if (*valid_moves_len < valid_moves_cap) {
        ...
        Serial.printf("WARN: valid_moves hit max size %d. Generated move was not added!\n", MAX_VALID_MOVES_CAPACITY);
    }
}
...
void getValidMovesForPawn(Move valid_moves[], int *valid_moves_len, int valid_moves_cap, Piece piece) {
    ...
    while (locate_piece_from(&piece_loc, piece, piece_loc.x, piece_loc.y + 1)) {
        ...
        switch (pieceColor) {
            ...
            case BLACK_PIECE:
                ...
                break;
            default:
                Serial.printf("ERROR: Not a valid pieceColor\n");
                break;
        }
    ...
    //     Serial.printf("WARN: Did not find a move for %s\n", get_piece_name(piece));
    // }
}
...
bool gen_queen_rook_moves(Move valid_moves[], int *valid_moves_len, int valid_moves_cap, Move move, Piece piece,
    ...
    if (other_piece == EMPTY && is_valid_pos_for_king(pieceColor, move.source, move.dest)) {
        ...
        if (other_piece == WHITE_KING || other_piece == BLACK_KING) {
            Serial.printf("WARN: A move involving taking over a king was generated\n");
        }
        add_move_into_moves(valid_moves, valid_moves_len, valid_moves_cap, move);
...
int kingIsInCheck(Piece kingPiece) {
    ...
    switch (oppKingColor) {
        ...
        default:
            Serial.printf("ERROR: UNREACHABLE CASE STATEMENT\n");
            break;
    }
    return 0;
}
    ...
    *valid_moves_len = 0;
    switch (piece) {
        ...
        case WHITE_KING:
            ...
            break;
        default:
            Serial.printf("ERROR: %s UNREACHABLE SWITCH CASE\n", get_piece_name(piece));
            break;
    }
    // Serial.printf("INFO: Valid Moves for %s:\n", get_piece_name(piece));
...
Move get_best_move(Move valid_moves[], int *valid_moves_len) {
    if (*valid_moves_len == 0) {
        Serial.printf("ERROR: No best move to find with zero moves\n");
    }
    int best_move_idx = 0;
    for (int i = 0; i < *valid_moves_len; ++i) {
...
int get_best_moves(Move put_best_into[], int put_best_into_size, Move valid_moves[], int valid_moves_len) {
    ...
    int copy_moves = min(put_best_into_size, valid_moves_len);
...
int chess_engine(Move to_return_moves[], int *to_return_moves_len, int to_return_moves_size, PieceColor color) {
    ...
    if (locate_piece_from(&kingPos, BLACK_KING, 0, 0) == 0) {
        Serial.printf("WARN: Could not locate the BLACK_KING\n");
    }
    Move moves[4] = {};
    int moves_len = 0;
    ...
    switch (color) {
        case BLACK_PIECE: {
            ...
            if (moves_len == 0) {
                Serial.printf("INFO: No moves for Black found. Checkmate\n");
                return 0;
            }
            break;
        }
        case WHITE_PIECE: {
            ...
            if (moves_len == 0) {
                Serial.printf("INFO: No moves for White found. Checkmate\n");
                return 0;
            }
            break;
        }
        ...
        for (int line = 0; line < 2; ++line) {
            for (int j = 0; j < BOARD_COLS; ++j) {
                ...
                if (line == 0) {
                    ...
                    Serial.printf("|    [%s]    | ", get_piece_name(curr_piece));
                }
            }
            Serial.printf("\n");
        }
